...
export const Route = createFileRoute("/_authenticated/admin/notifications")({
  head: () => ({
    meta: [
      { title: "Notifications — Admin — QEVRIX" },
      { name: "description", content: "System-wide notifications." },
  ...
  const q = useQuery({
    ...
    queryFn: async (): Promise<Notif[]> => {
      const { data } = await supabase
        .from("notifications")
        .select("id, type, message, is_read, created_at, recipient_user_id")
    ...
    <div className="space-y-6">
      ...
      <div className="card-surface p-0">
        {q.isLoading ? (
          ...
          <NotificationEmpty title="No notifications" />
        ) : (
          <ul className="divide-y divide-border">
            {items.map((n) => (
              <NotificationItem
                ...
                isRead={n.is_read}
                message={n.message}
                meta={<>{new Date(n.created_at).toLocaleString()} · {n.type ?? "info"}</>}
                actions={
                  <>
                    {!n.is_read && (
                      <button
