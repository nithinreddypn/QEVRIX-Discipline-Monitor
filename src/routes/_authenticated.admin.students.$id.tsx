...
import { ArrowLeft, Mail, Phone, UserRound, Pencil, Trash2, ShieldAlert, Camera } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/admin/students/$id")({
  head: () => ({
    meta: [
      { title: "Student Detail — Admin — QEVRIX" },
      { name: "description", content: "Administrator view of student details." },
...
function AdminStudentDetailPage() {
  ...
  const student = useQuery({
    ...
    queryFn: async (): Promise<Student | null> => {
      const { data } = await supabase
        .from("students")
        .select("id, full_name, usn, semester, email, phone, profile_photo_url, branch_id, status, rejection_reason, branches(name, code)")
        .eq("id", id)
        .maybeSingle();
  ...
  const detections = useQuery({
    queryKey: ["admin-student-detections", id],
  ...
  return (
    <div className="space-y-6">
      ...
      {student.isLoading ? (
        <div className="card-surface p-12 text-center text-sm text-muted-foreground">Loading student…</div>
      ) : !s ? (
        <div className="card-surface p-12 text-center text-sm text-muted-foreground">Student not found.</div>
      ) : (
        <>
          <section className="card-surface p-6">
              ...
              <div className="flex flex-wrap items-center gap-5">
                ...
                <div className="min-w-0">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
                    ...
                    {s.branches && <span>{s.branches.name} ({s.branches.code})</span>}
            ...
            {s.rejection_reason && (
              <div className="mt-4 p-3 rounded bg-secondary/50 border border-border text-xs text-muted-foreground max-w-2xl">
                <span className="font-semibold block mb-0.5 text-foreground">Status Details:</span>
                {s.rejection_reason}
              </div>
            )}
          </section>
                        ...
                        <tr key={r.id} className="transition-colors hover:bg-secondary/30">
                          ...
                          <td className="whitespace-nowrap px-6 py-3.5">
                            <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${
                              r.id_card_found ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-800"
                            }`}>
                              {r.id_card_found ? "Yes" : "No"}
                            </span>
                          </td>
                          <td className="whitespace-nowrap px-6 py-3.5">
                            <span className="capitalize">{r.status ?? "Processed"}</span>
                          </td>
                        </tr>
                      );
                    })}
          ...
          <Dialog open={isEditing} onOpenChange={setIsEditing}>
            <DialogContent className="max-w-lg">
              <DialogHeader>
                ...
                <DialogDescription>Modify student registration details directly.</DialogDescription>
              </DialogHeader>
              ...
              <DialogFooter className="mt-4">
                ...
                <button
                  disabled={updateStudent.isPending || !editForm.full_name || !editForm.usn}
            ...
            <DialogContent className="max-w-md">
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2 text-rose-600">
                  <ShieldAlert className="h-5 w-5" /> Delete Student
                </DialogTitle>
                <DialogDescription>
                  Are you sure you want to permanently delete student {s.full_name}? This action is irreversible.
                </DialogDescription>
              </DialogHeader>
              <DialogFooter className="mt-4">
